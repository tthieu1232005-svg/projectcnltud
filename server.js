...
// Bạn cần import model Branch ở phía trên file
const Branch = require("./models/Branch"); // Đảm bảo đường dẫn đúng

// --- Luồng Khách hàng (Customer) ---
app.get("/", async (req, res) => {
  try {
    // 1. Lấy tất cả chi nhánh từ database
    const branches = await Branch.find({}).lean();

    // 2. Truyền biến branches vào view
    res.render("customer/home", {
      branches: branches, // Đây là thứ đang thiếu
      scripts: '<script src="/js/customer-main.js"></script>',
    });
  } catch (error) {
    console.error("Lỗi khi lấy dữ liệu home:", error);
