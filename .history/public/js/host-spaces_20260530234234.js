document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!hostId || !token) {
        showToast("Vui lòng đăng nhập để truy cập trang điều hành!"); // Hoặc alert()
    ...
    try {
        const response = await fetch(`/api/hosts/${hostId}/dashboard-data`, {
            ...
            headers: {
                ...
                // Vì bạn đã có JWT Token, ta kẹp luôn vào Header để tăng bảo mật
        ...
        if (response.ok) {
            // Gọi hàm đắp dữ liệu ra UI mà ta đã viết ở bước trước
            renderDashboard(result);
        } else {
            console.error("Lỗi từ server:", result.error || result.message);
            // Nếu Token hết hạn, API trả về lỗi 401 thì cho đăng xuất
...
function logout() {
    localStorage.clear(); // Xóa sạch token, id, role...
