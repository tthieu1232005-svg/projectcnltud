...
// ID mẫu dùng kiểm tra dữ liệu từ MongoDB Compass (Thay bằng ID thực của bạn khi cần)
...
async function initHostDashboard(hostId) {
    try {
        ...
        // 1. Cập nhật các Tabs chọn chi nhánh động từ DB
        ...
        if (tabContainer) {
            ...
            result.branches.forEach(branch => {
                tabContainer.innerHTML += `
                    <button type="button" class="branch-tab px-5 py-2.5 rounded-xl text-sm font-bold transition" id="tab-${branch._id}" onclick="switchBranch('${branch._id}')">${branch.Name}</button>
                `;
            });
        }

        // 2. Đẩy dữ liệu lên 4 thẻ tổng quan trên giao diện
        ...
        // 3. Tái cấu trúc sơ đồ trạng thái phòng Live (Floor Plan)
        const floorPlanContainer = document.getElementById("host-floor-plan-mini");
        if (floorPlanContainer) {
            floorPlanContainer.innerHTML = ""; // Xóa các div tĩnh mẫu

            result.liveFloorPlan.forEach(space => {
                let bgClass = "bg-emerald-100 border-emerald-200/80 text-emerald-800"; // Mặc định Đang trống
                if (space.status === 'occupied') {
                    bgClass = "bg-red-100 border-red-200/80 text-red-800"; // Đang dùng
                } else if (space.status === 'maintenance') {
                    bgClass = "bg-amber-100 border-amber-200/80 text-amber-800"; // Bảo trì
                }
        ...
        // 4. Đổ dữ liệu lịch sử vào bảng "Booking gần nhất"
        const recentTableBody = document.getElementById("host-recent-table");
        if (recentTableBody) {
            recentTableBody.innerHTML = ""; // Xóa trống dòng mẫu
        ...
        // 5. Cập nhật khu vực Tài chính chi tiết ở chân trang
        ...
        if (financeSection) {
            financeSection.innerText = totalExpected.toLocaleString('vi-VN') + "đ";
        }
