...
async function loadProfile() {
    try {
        // Tạm thời mình dùng cách fetch API không token theo thiết lập hiện tại của bạn nhé
        // (Hoặc nếu bạn xài Token thì cứ giữ nguyên dòng lấy Token của bạn)
        const response = await fetch('/host/api/profile', {
            method: 'GET'
            // headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } // Mở comment nếu bạn xài token
        });
        ...
        // ĐIỀN DỮ LIỆU VÀO FORM (Đã xóa các dòng .innerText gây lỗi)
        // Dùng .value để gán giá trị vào thẻ <input>
