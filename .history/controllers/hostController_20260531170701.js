...
async function getHostDashboardStats(req, res) {
  try {
    // PHẢI KIỂM TRA: Nếu middleware chưa chạy hoặc chưa đăng nhập, không để sập server
    if (!req.user || !req.user._id) {
      return res.status(401).json({ error: "Vui lòng đăng nhập để xem số liệu." });
    }
    ...
    // 🔥 LOGIC QUAN TRỌNG: Nếu lọc theo từng Chi nhánh cụ thể
    ...
    // A. Tính Doanh thu thực tế thành công & Tiền treo
    ...
    // B. Tính Tổng đặt chỗ và Lượt khách duy nhất
    ...
    // C. Đếm số chỗ/phòng đang hoạt động
    ...
    // 3. Trả kết quả đồng bộ chuẩn xác
    return res.status(200).json({
      branches,
      stats: {
        revenue: finance.revenue,
        paidAmount: finance.revenue,
        pendingAmount: finance.pendingAmount, // 🌟 ĐÃ SỬA: Thay từ finance.pending thành finance.pendingAmount
        totalBookings,
        totalOccupiedGuests,
        activeRoomsCount
      },
...
module.exports = {
  ...
  getHostBookings,
  renderDashboardView,   
  getDashboardStatsAPI : getHostDashboardStats
};