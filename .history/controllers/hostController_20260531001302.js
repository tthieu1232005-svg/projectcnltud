...
async function getHostProfile(req, res) {
  try {
    ...
    if (!profile) {
      return res.status(404).send('Không tìm thấy hồ sơ host');
    }

    return res.render('host/profile', {
      profile,
...
async function getHostDashboardData(req, res, next) {
  try {
    ...
    // Thay đổi những dòng này cho khớp chính xác với Schema của bạn:
...
// Cuối file controllers/hostController.js
module.exports = {
