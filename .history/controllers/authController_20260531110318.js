...
async function changePassword(req, res) {
    try {
        ...
        // 1. Lấy userId từ middleware xác thực (authMiddleware) truyền qua
        // Lưu ý: Phải chắc chắn route này có đi qua middleware check token
        ...
        // 4. So sánh mật khẩu cũ nhập vào với mật khẩu hiện tại trong DB
        const isMatch = await bcrypt.compare(String(oldPassword), user.PasswordHash);
        if (!isMatch) {
            return res.status(400).json({ error: 'Mật khẩu cũ không chính xác!' });
        }

        // 5. Hash mật khẩu mới và cập nhật vào DB
        const salt = await bcrypt.genSalt(10);
        ...
        // 6. Trả về thông báo thành công THẬT cho Frontend
