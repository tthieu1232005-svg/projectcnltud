...
async function getDashboard(req, res) {
  try {
    ...
    // 0. BẮT BUỘC ÉP KIỂU SANG OBJECTID (Sửa lỗi Mongoose không tự ép kiểu)
    const objectIdHost = new mongoose.Types.ObjectId(hostId);

    // 1. LẤY THÔNG TIN PROFILE HOST (Đổi HostID thành UserID)
    const hostProfile = await HostProfile.findOne({ UserID: objectIdHost }).lean() || { name: 'Chủ cơ sở mới' };

    // 2. LẤY DANH SÁCH CHI NHÁNH VÀ KHÔNG GIAN (Truyền objectIdHost thay vì chuỗi)
    ...
    const recentBookings = await Booking.find({ SpaceID: { $in: spaceIds } })
      .populate('CustomerID', 'FullName')  // Sửa 'name' thành 'FullName'
